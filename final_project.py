...
if uploaded_file is not None:
    try:
        ...
        with tab4:
            ...
            PROMPTS = {
                "Run Analysis": "Analyze the dataset and Identify the data types, count missing values, and summarize the top 3 most significant statistical insights or correlations found in the data and show using python and streamlit.",
                "Fix Dataset": "Clean the dataset by performing the following: 1. Fill or drop missing values appropriately. 2. Remove duplicate rows. 3. Standardize column names to snake_case. 4. Detect and flag any extreme outliers in numerical columns.",
                "Pivot Table": "Create a pivot table that aggregates the main numerical columns by the most relevant categorical columns. Show the sum and average for each category to provide a multi-dimensional summary of the dataset.",
                "Dashboard Generation": "generate a full  static business analysis dashboard using the whitelisted liberis and use python and Streamlit to show the result and do not show any error.",
                "Trend Analysis": "Identify the primary time-series column and a numerical value. Plot a line chart showing the trend over time. Highlight any significant spikes, dips, or seasonality observed in the data.",
                "Forecast": "Perform a simple linear regression or time-series forecast. Based on the historical trends in the data, predict the values for the next 5 time periods and visualize the 'Actual vs. Predicted' values in a chart."
            }
            ...
            config = {
                ...
                "custom_whitelisted_dependencies": [
                    "streamlit", "pandas", "numpy", "matplotlib", "seaborn", "Pillow"
                ]
            }
            ...
            if df is not None:
                agent = Agent(
                    df,
                    description="You are a data analysis agent. Your main goal is to help non-technical users to analyze data, visualize, storytelling about the data, generating insights, dashboard generation and also other help they need.CRITICAL INSTRUCTIONS: You must write a Python script using pandas to answer this. Always wrap your code in standard markdown ```python ``` blocks. Do not provide conversational text. ",
                    config=config
                )
                ...
                if st.button(f"Generate {selected_task}"):
                    with st.spinner(f"Running {selected_task}..."):
                        current_prompt = PROMPTS[selected_task]
